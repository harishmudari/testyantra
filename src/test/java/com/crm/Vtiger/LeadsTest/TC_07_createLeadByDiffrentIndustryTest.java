...
public class TC_07_createLeadByDiffrentIndustryTest extends BaseClass {
	...
	String LASTNAME = eLib.getDataFromExcel("testCase", 80, 5);
	clp.lastNameEdt(LASTNAME+jvLib.getRandomNumber(100));
	String COMPANYNAME = eLib.getDataFromExcel("testCase",80, 6);
	clp.companyNameEdt(COMPANYNAME);
	
	//select industry with "Apparel" menu
	String INDUSTRYNAME = eLib.getDataFromExcel("testCase", 80, 7);
	clp.industryDrpDwn(INDUSTRYNAME);
	...
	// select search in dropDown menu as lead no
	 String drpDwnMenuName = eLib.getDataFromExcel("testCase", 80, 8);
...
//	wLib.waitForElementToBeClickable(driver, waitForIt);
	...
	System.out.println("New Lead created with a diffrent Industry "+"\n"+"and a unique auto generated LEAD NUMBER--> "+actulValue);
	}	
}
