...
public class TC_08_createLeadByDiffrentLeadStatusTest extends BaseClass{
	...
	String LASTNAME = eLib.getDataFromExcel("testCase", 92, 5);
	clp.lastNameEdt(LASTNAME+jvLib.getRandomNumber(100));
	String COMPANYNAME = eLib.getDataFromExcel("testCase",92, 6);
	clp.companyNameEdt(COMPANYNAME);
	
	//select lead status with "Attempted to Contact" menu
	String LEADSTATUSNAME = eLib.getDataFromExcel("testCase", 92, 7);
	clp.leadStatusDrpDwn(LEADSTATUSNAME);
	...
	// select search in dropDown menu as lead no
	 String drpDwnMenuName = eLib.getDataFromExcel("testCase", 92, 8);
	lp1.selectFromSearchInDrpDwnMenu(drpDwnMenuName);
	
	// click on search now button
	lp1.clickOnSearchNow();
	
//	//wait for element to be ready with out thread.sleep 
//	WebElement waitForIt = lp1.getLeadNoInLeadMainPage();
//	wLib.waitForElementToBeClickable(driver, waitForIt);
	...
	System.out.println("New Lead created with a diffrent lead Status "+"\n"+"and a unique auto generated LEAD NUMBER--> "+actulValue);
	}	
}
