...
public class TC_09_createLeadByDiffrentRatingTest extends BaseClass{
	...
	String LASTNAME = eLib.getDataFromExcel("testCase", 104, 5);
	clp.lastNameEdt(LASTNAME+jvLib.getRandomNumber(100));
	String COMPANYNAME = eLib.getDataFromExcel("testCase",104, 6);
	clp.companyNameEdt(COMPANYNAME);
	
	//select lead status with "Attempted to Contact" menu
	String RATINGNAME = eLib.getDataFromExcel("testCase", 104, 7);
	clp.ratingDrpDwn(RATINGNAME);
	...
	// select search in dropDown menu as lead no
	 String drpDwnMenuName = eLib.getDataFromExcel("testCase", 104, 8);
...
//	wLib.waitForElementToBeClickable(driver, waitForIt);
	...
	System.out.println("New Lead created with a diffrent Rating "+"\n"+"and a unique auto generated LEAD NUMBER--> "+actulValue);
